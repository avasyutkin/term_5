...
mlp = MLPClassifier(solver='adam', activation='tanh', random_state=0, hidden_layer_sizes=[100], alpha=0.0001)
mlp.fit(points_train, labels_train)
for i in mlp.predict_proba(points_train):
    print("[ {:.2f}".format(i[0]), "{:.2f}".format(i[1]), "]")

prediction = mlp.predict(points_test)

print("Правильность на тестовом наборе: {:.2f}".format(mlp.score(points_test, labels_test)))
...
prediction = mlp.predict(points_new_after)
...
mlp = MLPClassifier(solver='lbfgs', activation='tanh', random_state=0, hidden_layer_sizes=[10, 10, 10, 5], alpha=0.001).fit(X_train, y_train)
mglearn.plots.plot_2d_separator(mlp, X_train, fill=True, alpha=.3)
mglearn.discrete_scatter(X_train[:, 0], X_train[:, 1], y_train)
plt.xlabel("Признак 0")
plt.ylabel("Признак 1")
plt.show()
for i in mlp.predict_proba(X_test):
    print("[ {:.2f}".format(i[0]), "{:.2f}".format(i[1]), "]")
